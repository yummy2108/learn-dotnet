...
namespace SqliteDal
{
    public class SqliteScmContext : IScmContext
    {
        ...
        public void DeletePartCommand(int id, DbTransaction transaction) {
            connection.Execute(@"DELETE FROM PartCommands WHERE Id=@Id", new {Id = id}, transaction);
        }
        ...
        public void UpdateInventoryItem(int partTypeId, int count, DbTransaction transaction) {
            connection.Execute(@"UPDATE InventoryItem
                ...
                new { Count = count, PartTypeId = partTypeId}, transaction);
        }
        ...
        public void CreateOrder(Order order) {
            ...
            try {
                order.Id = connection.Query<int>(
                    ...
                    VALUES 
                    (@SupplierId, @PartTypeId, @PartCount, @PlacedDate);
                    SELECT last_insert_rowid();", order, transaction
                ).First();
